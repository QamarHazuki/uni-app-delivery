...
import { getHotRecommendAPI } from '@/service/hot'
import type { SubTypeItem } from '@/types/hot.d'
import { onLoad } from '@dcloudio/uni-app'
import { ref } from 'vue'
import customTopbar from '@/components/customTopbar.vue'

const { safeAreaInsets } = uni.getSystemInfoSync();
const navHeight = `calc(80rpx  + ${safeAreaInsets?.top ?? 0}px)`
    ...
    <view class="viewport">
        ...
        <view class="tabs">
            <text v-for="(item, index) in subTypes" :key="item.id" class="text"
                :class="{ active: index === activeIndex }" @tap="activeIndex = index">{{ item.title }}</text>
        </view>
        <!-- 推荐列表 -->
...
.scroll-view {
    flex: 1;
    height: calc(100vh - var(--nav-h) - 100rpx - 180rpx - var(--tabbar-h));
}
