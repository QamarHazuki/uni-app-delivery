...
import { postLoginAPI, postLoginWxMinAPI, postLoginWxMinSimpleAPI } from '@/service/login.ts'
...
const loginSuccess = (profile: LoginResult) => {
  ...
  setTimeout(() => {
    ...
    if (pages.length > 1) {
      ...
      // uni.switchTab({ url: '/pages/food/index' })      // tabBar 首页
      uni.reLaunch({ url: '/pages/food/index' })         // 非 tabBar 或强制回首页
    }
  }, 500)
}
  ...
  <view class="viewport">
    ...
    <view class="login">
      ...
      <view class="tips" :class="{ animate__shakeY: isAgreePrivacyShakeY }">
        <label class="label">
          <radio class="radio" color="#28bb9c" :checked="isAgreePrivacy" @tap="isAgreePrivacy = !isAgreePrivacy" />
          <text @tap="isAgreePrivacy = !isAgreePrivacy">登录/注册即视为你同意。。。</text>
        </label>
